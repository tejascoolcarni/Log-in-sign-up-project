...
class login:
    def __init__(self,root):
        ...
        label2 = Label(frame_signup, text="Email or Username ", font=("Goudy old style", 20, "bold"),
        ...
        def login_val(event=None):
            ...
            find_user = ("SELECT * FROM data WHERE email=? or username =? and password=?")
            c.execute(find_user, [(v_email.get()), (v_email.get()), (v_password.get())])
            row_e = c.fetchall()
        ...
        e_email = Entry(frame_signup, textvariable=v_email, font=("times new roman", 15, "bold"),
                               bg='lightgray')
        e_email.place(x=30, y=195, width=270, height=35)

        label3 = Label(frame_signup, text="Password", font=("Goudy old style", 20, "bold"),
                       fg='orangered', bg='white')
