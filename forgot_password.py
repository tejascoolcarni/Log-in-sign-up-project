    ...
    def __init__(self,root):
        ...
        def reset(event=None):
            ...
            if v_password.get() == "":
                ...
                conn = sqlite3.connect('attempt6.db')
                c = conn.cursor()
                find_user = ("SELECT * FROM data WHERE security_a = ? and email=? or username =? or phone=?")
                c.execute(find_user, [(v_answer.get()),(v_credential.get()), (v_credential.get()), (v_credential.get())])
                verify = c.fetchall()
                # print(verify[0][8])
        ...
        credentials = Entry(frame_forgot_password, textvariable=v_credential, font=("times new roman", 15, "bold"), bg='lightgray')
        ...
        answer = Entry(frame_forgot_password, textvariable= v_answer, font=("times new roman", 15, "bold"), bg='lightgray')
        answer.place(x=50, y=400, width=270, height=35)

        new_password = Entry(frame_forgot_password, textvariable=v_password, font=("times new roman", 15, "bold"), bg='lightgray')
        new_password.place(x=50, y=460, width=270, height=35)

        con_new_password = Entry(frame_forgot_password, textvariable=v_con_password, font=("times new roman", 15, "bold"), bg='lightgray')
